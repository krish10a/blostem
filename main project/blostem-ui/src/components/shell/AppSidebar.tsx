...
export function AppSidebar({
  ...
  return (
    <>
      ...
      <nav
        className={cn(
          "fixed inset-y-3 left-3 z-50 flex w-[17.5rem] flex-col overflow-hidden rounded-[28px] border border-white/8 bg-[#0b1326]/88 p-4 shadow-[0_35px_100px_rgba(6,14,32,0.75)] backdrop-blur-2xl transition-transform duration-300 lg:inset-y-4 lg:left-4",
      ...
      >
        ...
        <div className="mb-5 rounded-2xl border border-white/8 bg-gradient-to-br from-primary/12 via-white/4 to-secondary/8 px-4 py-4">
        ...
        <div className="thin-scrollbar flex-1 space-y-1 overflow-y-auto px-2 pb-4">
          {CORE.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={onClose}
                className={cn(
                  "group haptic-hover flex items-center gap-3 rounded-2xl px-3 py-3 font-label text-sm tracking-wide transition-all",
                  isActive
        ...
        <div className="mt-4 space-y-3 px-2 pt-4">
          ...
          <Link
            href="/billing"
            onClick={onClose}
            className="flex items-center gap-3 rounded-2xl px-3 py-2 text-slate-500 transition-colors hover:bg-white/6 hover:text-slate-200"
          >
